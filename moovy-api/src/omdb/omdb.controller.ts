...
import {OmdbDto, OmdbQueryParameters, OmdbSearchDto} from './omdbDto';
...
export class OmdbController {
  ...
  @Get()
  findAllMovies(@Query() queryParameters: OmdbQueryParameters, @Req() request: any): Promise<OmdbSearchDto> {
    const user: LoggedUserDto = plainToInstance(LoggedUserDto, request.user);
